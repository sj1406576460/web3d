...
document.writeln(
	"<div class=\"tax-tools\"><button class=\"tax-play\"onclick=\"tax_auto_paly(); return false\"style=\"display:none;\">开始</button><button class=\"tax-stop\"onclick=\"crl.stop(); return false\">停止</button><button id=\"btn\"class=\"tax-zoom\"onclick=\"crl.stop(); return false\">放大宝贝</button></div>"
	);
document.writeln(
...
function tax_auto_paly() {
	clearTimeout(t);
	//crl.play()
}
$(document).ready(function() {
	$(".tax-stop").click(function() {
		$(".tax-play").show();
		$(".tax-stop").hide()
	});
	$(".tax-play").click(function() {
		$(".tax-stop").show();
		$(this).hide()
	});
	  
	$("#circlr img").dblclick(function(){
		//$(this).css('width',$(this).width()*1.2)
		//$(this).css('height',$(this).height()*1.2)
	})
	  
	$("#circlr").on("touchstart || mousedown", function() {
		clearTimeout(t);
		$(".tax-play").show();
		$(".tax-stop").hide()
	});
	$("#circlr").on("touchend || mouseup", function() {
		t = setTimeout("tax_auto();", Showec_Config_mkj005.Showec_Delay * 1000)
	})
});
