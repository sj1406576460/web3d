				...
				if(group.children.length==0){
					...
					if(model!=null){
						for(i=0;i<items.length;i++){
							debugger
							  if(i == 0){//最左边
								if(model.right == true && items[i].left == true){
									//最左边可以加
									addPlusList.push({x:group.children[i].x,stlId:group.children[i].stlId})
									//addModel(model,2)
						        }
						       }
							    ...
							    if(model.left === true && model.right == true){
									if(i>0 && i<items.length-1) { //有中间的情况
						            // i和i~当前组合.length-1之间的位置可以加
									    let  it=addPlusList.find((item)=>{
											return item.stlId=model.stlId
										});
							 ...
							 if(i == items.length-1){ //最右边
							 	if(model.left == true && items[i].right == true){
							        //最右边可以加
									addPlusList.push({x:group.children[i].x,stlId:group.children[i].stlId})
									//addModel(model,1)
							    }
